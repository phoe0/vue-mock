<template>
  <div class="content">
	  <!-- (顶部的返回按钮) 灰色、透明、去边框 -->
    <uc-nav bg-color="transparent" :border-width="0" arrow="gray"/>
    ...
    <div class="footbox">
       <!-- 登录按钮 -->
	   <uc-button @click="login" style="margin-top: .23rem">登录</uc-button>
		...
		login(){
			...
			this.$axios({
				...
				res => {
					...
					if(res.data.err==0){  // 如果登录成功,
						// 在跳转之前, 设置 localstorage
						window.localStorage.setItem('user',JSON.stringify(res.data))
						if(!this.$route.query.p){  // 如果跳转之前
